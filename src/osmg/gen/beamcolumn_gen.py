...
class BeamColumnGenerator:
    ...
    def add_vertical_active(
        ...
        for key in lvls.active:
            ...
            for i in range(n_sub):
                if i == 0:
                    n_i = top_node
                    o_i = offset_i + sec_offset_global
                else:
                    n_i = intnodes[i-1]
                    o_i = np.zeros(3)
                if i == n_sub - 1:
                    n_j = bottom_node
                    o_j = offset_j + sec_offset_global
                else:
                    n_j = intnodes[i]
                    o_j = np.zeros(3)
                ...
                if element_type.__name__ == 'elasticBeamColumn':
                    ...
                    component.force_beamcolumn_elements.add(element)
                else:
                    raise TypeError(
    ...
    def add_horizontal_active(
        ...
        for key in lvls.active:
            ...
            # instantiate a component assembly
            component = ComponentAssembly(self.model.uid_generator.new('component'), lvl.components)
            # fill component assembly
