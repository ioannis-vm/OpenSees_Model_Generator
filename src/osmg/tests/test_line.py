...
class TestLine:
    ...
    def test_direction(self):
        ...
        line = Line('l1', np.array([0, 0]), np.array([2, 2]))
        np.testing.assert_allclose(line.direction(), np.array([0.70710678, 0.70710678]))

    def test_intersect(self):
        """Test the intersection of two lines."""
        line1 = Line('l1', np.array([0, 0]), np.array([2, 2]))
        line2 = Line('l2', np.array([1, 0]), np.array([1, 3]))
        np.testing.assert_allclose(line1.intersect(line2), np.array([1.0, 1.0]))
    ...
    def test_intersects_pt_zero_length(self):
    ...
    def test_point_distance(self):
        ...
        assert line.point_distance(np.array([4, 2])) == pytest.approx(1.4142135623730951)
    ...
    def test_project(self):
        ...
        np.testing.assert_allclose(line.project(np.array([5, 0])), np.array([5.0, 0.0]))
        np.testing.assert_allclose(line.project(np.array([5, 5])), np.array([5.0, 0.0]))
