...
class TestEdge:
    ...
    def test_other_vertex(self):
        ...
        with pytest.raises(ValueError, match="The edge is not connected to the given vertex"):
...
class TestHalfedge:
    ...
    def test_halfedge_initialization(self):
    ...
    def test_halfedge_repr(self):
        """Test the string representation of a Halfedge."""
        h1 = Halfedge(self.v1, self.edge)
        h2 = Halfedge(self.v2, self.edge)
        h1.nxt = h2
        assert repr(h1) == f"(H{h1.uid} from E{h1.edge.uid} to E{h2.edge.uid} next H{h2.uid})"

    def test_halfedge_direction(self):
