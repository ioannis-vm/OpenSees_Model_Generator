...
from osmg.analysis.common import PointMass
...
for level in ('1', '2', '3', '4'):
    ...
    load_case_registry.modal['modal'].define_rigid_diaphragm(frame, parent_node)
...
basic_force_configuration = BasicForceConfiguration(
    ...
    step=0,
    force_to_length_factor=1.0e-4,
    moment_to_length_factor=1.0e-5,
)
fig = Figure3D(Figure3DConfiguration(ndm=2))
...
fig.add_supports(
    frame.nodes, load_case_registry.modal['modal'].fixed_supports, symbol_size=12.00
)
# fig.add_udl(
